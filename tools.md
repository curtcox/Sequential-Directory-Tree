...
**Status:** Proposal
**Date:** 2026-06-16
**Relates to:** [sdt-spec.md](sdt-spec.md) v1.0
...
| `sdt compact` | entries + `.0` | renumber covered entries to dense `1..N` | §3.6 r5, G6 |
...
- **classifier** — the five-rule covered/extra/sidecar decision (§3.6), including
  the ten-missing-predecessors rule (rule 5), which is the one place ad-hoc tools
  get classification wrong.
...
| `--validate` | exit `0` iff every argument is a valid index of `--kind` (§3.5) |

```
sdt code aa                  → 27          # base-26 inferred from lowercase
sdt code -k dir 27           → R           # ordinal → dir index
...
| Flag | View |
|------|------|
| *(none)* | classify each entry: `sidecar` / `file` / `dir` / `extra`, with decoded ordinal and the rule that made an extra |
| `--stat` | the nine §4.2 fields for this node, **derived from present state** (§5) — what a correct `.0` would contain, regardless of whether one exists |
| `--gaps` | the vacant ordinals behind `last_file` / `last_dir`; the `missing_*` measure broken out as a list, not just a count |
| `--fragile` | covered entries near rule-5 reclassification: how many of the ten predecessor slots below each are vacant, so you can see which entries one deletion would silently turn into extras |
| `--kind file\|dir` | restrict any view to one kind |

```
sdt read --stat            # the would-be sidecar for cwd
sdt read --fragile -r      # subtree-wide rule-5 risk scan
```

`--fragile` has no analog in standard filesystem tools and exists because rule 5
...
| `--portability` | flag §6 hazards: Windows reserved names once the sequence reaches `aux`/`AUX`/`CON`/`PRN`/`NUL` (§6.3); case-fold collisions like file `aa` vs dir `AA` (§6.2); nodes over GitHub's 3,000 combined-entry soft cap or ext4's 64,000 subdir hard cap (§6.1) |
...
for the requested kind. Read-only by default; `--create` materializes them.
...
```
sdt name -k dir                 # next directory index here
...
## `sdt compact` — restore density (§3.6 rule 5, G6)

Renumbers present covered entries of a kind so they form the dense prefix `1..N`,
driving `missing_*` to 0 and pulling fragile entries out of rule-5 range. Renames
covered entries and rewrites affected `.0`s; this is a real writer, so it defaults
to `--dry-run`-grade caution.
...
verb link them — that alone guarantees they all agree on rule 5.
...
3. `sdt sidecar`, `sdt name` — the maintenance core (the two originally-requested
   tools).
4. `sdt compact` — manages the format's sharpest edge (rule-5 fragility).
5. `sdt pack` — scaling/interchange, once the core is solid.
